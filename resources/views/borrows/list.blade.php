            ...
            <div class="container">
                @if($myBorrows[$i]->book_id === $books[$i]->id)
                    <table class="table col-md-8 col-sm-8 col-xs-10">
                        ...
                        <tbody>
                        <tr>
                            <th class="col-md-2.4 col-sm-2.4 col-xs-2">{{ $books[$i]->title }}</th>
                            <th class="col-md-2 col-sm-1.2 col-xs-1.2"> {{ $books[$i]->author }}</th>
                            <th class="col-md-2.4 col-sm-2.4 col-xs-2">{{date('d/m/Y',
                            strtotime($myBorrows[$i]->created_at))}}</th>
                            <th class="col-md-2.4 col-sm-2.4 col-xs-2">{{date( 'd/m/Y',
                        ...
                        <th class="col-xl-2 col-lg-2 col-md-2.4 col-sm-1.2 col-xs-0.8">
                            <form class="w-10" action="/books/borrows/update/{{ $myBorrows[$i]->id }}"
                                  method="POST">
                                @csrf
                                @method('GET')
                        ...
                        <th class="col-xl-2 col-lg-2 col-md-2.4 col-sm-1.2 col-xs-0.8">
                            <form class="w-10" action="/books/borrows/delete/{{ $myBorrows[$i]->id }}"
                                  method="POST">
                                @csrf
                                @method('DELETE')
